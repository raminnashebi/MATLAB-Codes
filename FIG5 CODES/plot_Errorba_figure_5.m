%% Ploting Errorbar for FIgure 5a

% Required Matrixes : 1) Totalcases matrix which calculated
%                        for 500 simulation with 9 different transmission ratio 
%                     2) Percentage reduction of transmission probability
%                        vector

for i=1:9
SEM1=std(TV_r1(:,i))/sqrt(length(TV_r1(:,i)))% TV_r1 is totalcase matrix for 500 simulation with 9 different transmission ratio 
ts1 = tinv([0.025  0.975],length(TV_r1(:,i))-1);  
CI1 = mean(TV_r1(:,i)) + ts1*SEM1;  % confidence interval of Totalcase
CIdist1(i,:)=abs(CI1-mean(TV_r1(:,i)));% Error bar
x(:,i)=mean(TV_r1(:,i),1);% Average of totalcase for each trasmision rate
end
CIdist1=CIdist1(1:end,:);
x=x';
shadedErrorBar(red_beta(1:9),x,CIdist1,'lineprops','-k','patchSaturation',0.10)
hold on 
x=[];
CIdist1=[];


%% Ploting Errorbar  Figure 5b, 5c, and 5d
%
% We plot infection ratio of household, workplace, social environment for 3
% scenarios: no stay-at-home restriction, stay-at-home restriction on sunday, 
% and stay-at-home restriction on saturday and sunday. we repeat the below
% codes for each scenarios.
%
